...
export const fmtLong = (iso: string | null | undefined, fallback = "—"): string =>
  safe(iso, (d) => d.toLocaleDateString(LOCALE, { day: "2-digit", month: "long", year: "numeric" }), fallback);
...
export const fmtDateTimeLong = (iso: string | null | undefined, fallback: string | null = null): string | null => {
  if (!iso) return fallback;
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return fallback;
