...
interface RailwayLog { id?: string; message: string; timestamp?: string; severity?: string; deploymentId?: string }
...
export default function InspectorPanel() {
  ...
  // railway log filters
  const [rwLogStatusFilter, setRwLogStatusFilter] = useState<"all" | "error" | "warn" | "info">("all");
  ...
  const railwayLogs = useMemo<RailwayLog[]>(() => {
    ...
    else if (raw && typeof raw === "object" && "logs" in raw && Array.isArray(raw.logs)) arr = raw.logs;
    return arr.map((l): RailwayLog => {
      ...
      if (l && typeof l === "object") {
        ...
        return {
          ...
          timestamp: typeof o.timestamp === "string" ? o.timestamp : typeof o.createdAt === "string" ? o.createdAt : undefined,
          severity: typeof o.severity === "string" ? o.severity : typeof o.level === "string" ? o.level : undefined,
  ...
  const errorCount = useMemo(() => railwayLogs.filter((l) => /err|fatal|crit/i.test(l.severity ?? "") || /err|fatal|exception|traceback/i.test(l.message)).length, [railwayLogs]);
  const warnCount = useMemo(() => railwayLogs.filter((l) => /warn/i.test(l.severity ?? "") || /warn/i.test(l.message)).length, [railwayLogs]);
        ...
        <div className="flex flex-col gap-1.5 border-b border-border/30 px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            {(["all", "error", "warn", "info"] as const).map((s) => (
              <FilterChip
                key={s}
                label={s === "all" ? "alle" : s === "error" ? `Fehler${errorCount ? ` (${errorCount})` : ""}` : s === "warn" ? `Warn${warnCount ? ` (${warnCount})` : ""}` : "Info"}
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground/70">Keyword</span>
            <input
              value={rwLogKeyword}
            ...
            />
            {rwLogKeyword && (
              <button onClick={() => setRwLogKeyword("")} className="text-[9px] text-muted-foreground hover:text-foreground">×</button>
            )}
          </div>
        ...
        {active === "railway" && railwayLogs.length > 0 ? (
          <div className="flex flex-col gap-1">
            {filteredRailwayLogs.length === 0 && (
              <div className="text-muted-foreground/40 text-center py-6 text-[10px]">Keine Logs passen zum Filter</div>
            )}
...
function FilterChip({ label, active, severity, onClick }: { label: string; active: boolean; severity?: "error" | "warn" | "info"; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        ...
        active
          ? severity === "error" ? "border-red-400/60 bg-red-500/15 text-red-300"
            : severity === "warn" ? "border-amber-400/60 bg-amber-500/15 text-amber-300"
            : severity === "info" ? "border-sky-400/60 bg-sky-500/15 text-sky-300"
            : "border-border/60 bg-muted/40 text-foreground"
...
function LogLine({ log, keyword }: { log: RailwayLog; keyword: string }) {
  ...
  const borderClass = isError ? "border-l-2 border-l-red-400/60" : isWarn ? "border-l-2 border-l-amber-400/60" : "border-l-2 border-l-transparent";
  ...
  return (
    <div className={cn("flex gap-2 px-1 py-0.5 rounded-sm", bgClass, borderClass)}>
      ...
      <span className="whitespace-pre-wrap break-all">
        {parts.map((p, i) => (
          <span key={i} className={p.highlight ? "bg-yellow-300/25 text-yellow-200 font-medium" : ""}>
            {p.text}
          </span>
        ))}
      </span>
