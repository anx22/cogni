// =============================================================================
//  EntityComposer — Unified Composer (Phase 5, kontinuierlicher Morph).
//  EINE Shell, die sich von der ruhenden Leiste fließend ins Panel verwandelt
//  (kein Hart-Wechsel). Ersetzt HomePrompt + InputOverlay + FacePill-2×2.
//  Auto-Detect Text↔Link; Datei/Sprache als Affordanzen. Orb-optional.
//  Spec: docs/entity-core.md §Composer.
// =============================================================================
...
const EntityComposer = ({
  ...
  return (
    <div
      className={cn("entity-composer-shell", c.open && "is-open", className)}
      onPaste={c.handleRootPaste}
    >
      {/* Ruhende Leiste — morpht weg beim Öffnen */}
      <div className="composer-resting">
        <button
          type="button"
      ...
      <div className="composer-panel">
        <div className="relative">
          ...
          {c.mode === "text" && (
            <div className={cn("space-y-3", !contextHint && "pt-9")}>
              {c.pastePreview ? (
                <>
          ...
          {c.mode === "voice" && (
            <div className="flex min-h-[120px] flex-col items-center justify-center gap-4 rounded-md border border-dashed border-border/30 bg-background/20 mt-9">
              {c.voice.status === "idle" && (
                <button
                  type="button"
              ...
              {c.voice.status === "error" && (
                <div className="flex flex-col items-center gap-3 text-destructive/80">
                  <p className="text-xs tracking-wide">
                    {c.voice.error ?? "Fehler bei der Aufnahme"}
                  </p>
                  <button
                    type="button"
                    onClick={c.voice.reset}
