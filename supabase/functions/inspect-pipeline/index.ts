...
Deno.serve(
  withErrorBoundary("inspect-pipeline", async (req) => {
    ...
    log.stage("start", "request", {
      has_asset: !!body.asset_id,
      has_project: !!body.project_id,
      has_correlation: !!body.correlation_id,
    ...
    try {
      if (body.asset_id) {
        ...
        if (parsedIds.length) {
          const { data } = await sb
            .from("proposed_facts")
            .select(
              "id, status, fact_type, delta_type, confidence, created_at, graphiti_episode_uuid",
            )
        ...
        if (proposed.length) {
          ...
          const [{ data: cases }, { data: facts }] = await Promise.all([
            sb
              .from("review_cases")
              .select("*")
              .in("proposed_fact_id", proposedIds)
              .eq("user_id", userId),
            sb
              .from("canonical_facts")
              .select(
                "id, fact_type, source_proposed_fact_id, valid_from, valid_until, superseded_by, graphiti_uuid",
              )
              .in("source_proposed_fact_id", proposedIds)
              .eq("user_id", userId),
          ]);
      ...
      if (body.project_id) {
        const [{ data: snapshot }, { data: events }, { data: facts }] = await Promise.all([
          ...
          sb
            .from("canonical_facts")
            .select("id, fact_type, graphiti_uuid, valid_from, valid_until, superseded_by")
            .eq("project_id", body.project_id)
            .eq("user_id", userId)
            .limit(200),
        ]);
        ...
        trace.graphiti_coverage = facts
          ? {
              total: facts.length,
              with_uuid: facts.filter((f: { graphiti_uuid: string | null }) => f.graphiti_uuid)
                .length,
              active: facts.filter(
                (f: { valid_until: string | null; superseded_by: string | null }) =>
