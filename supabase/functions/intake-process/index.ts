...
Deno.serve(
  withErrorBoundary("intake-process", async (req) => {
    ...
    try {
      ...
      // Status: processing
      await admin.from("assets").update({ processing_status: "processing" }).eq("id", asset_id);
      ...
      // Status: completed
      await admin.from("assets").update({ processing_status: "completed" }).eq("id", asset_id);
      ...
      log.stage("exit", "parsed + chain dispatched", {
        segments_count: Array.isArray(segments) ? segments.length : 0,
      });
      await log.flush();
      return new Response(
        JSON.stringify({ ok: true, segments_count: Array.isArray(segments) ? segments.length : 0 }),
        {
