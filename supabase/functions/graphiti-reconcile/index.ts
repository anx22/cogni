...
interface Payload {
  ...
  last_n?: number;
  max_facts?: number;
  /** Zusätzlich failed-Sync-Logs erneut reconcilen (K4). */
  reconcile_failed?: boolean;
  /** Substring-Match auf normalisierte Reason (K4). */
  failed_reason_filter?: string;
  /** Maximale Retry-Versuche (default 3). */
  max_attempts?: number;
  /** Cooldown zwischen Retrys in Minuten (default 30). */
  retry_cooldown_min?: number;
}

Deno.serve(
  withErrorBoundary("graphiti-reconcile", async (req) => {
    ...
    try {
      ...
      log.stage("scan", "search pending facts", {
        lastN,
        maxFacts,
      ...
      // K4: failed-Sync-Log Retry-Pfad. Lädt failed-Rows, filtert per
      // selectRetryCandidates (attempt/cooldown/reason), und merged die
      // zugehörigen canonical_facts in `pending`, sodass der bestehende
      // Episode-Match-Loop sie automatisch mit-bearbeitet.
      let retryStats = { eligible: 0, skipped_attempt: 0, skipped_cooldown: 0, skipped_reason: 0 };
      if (body.reconcile_failed) {
        const failedQ = admin
          .from("graphiti_sync_log")
          .select("entity_id, attempt, error, last_retry_at")
          .eq("status", "failed")
          .limit(maxFacts);
        const { data: failedRows, error: fErr } = await failedQ;
        if (fErr) {
          log.warn("retry", "failed_sync_log query error", { error: fErr.message });
        } else {
          const decision = selectRetryCandidates((failedRows ?? []) as FailedSyncRow[], {
            max_attempts: body.max_attempts,
            retry_cooldown_min: body.retry_cooldown_min,
            failed_reason_filter: body.failed_reason_filter,
          });
          retryStats = {
            eligible: decision.eligible.length,
            skipped_attempt: decision.skipped_by_attempt,
            skipped_cooldown: decision.skipped_by_cooldown,
            skipped_reason: decision.skipped_by_reason,
          };
          const retryIds = decision.eligible.map((r) => r.entity_id);
          if (retryIds.length > 0) {
            // CFs für diese IDs nachladen (user-scoped wenn nicht service-role).
            let retryCfQ = admin
              .from("canonical_facts")
              .select("id, project_id, fact_type, user_id, provenance")
              .in("id", retryIds);
            if (userId) retryCfQ = retryCfQ.eq("user_id", userId);
            const { data: retryCfs, error: rcErr } = await retryCfQ;
            if (rcErr) {
              log.warn("retry", "canonical_facts lookup error", { error: rcErr.message });
            } else {
              const have = new Set((pending ?? []).map((cf) => cf.id));
              for (const cf of retryCfs ?? []) {
                if (!have.has(cf.id)) {
                  (pending ?? []).push(cf);
                }
              }
              // last_retry_at + attempt+1 vorab schreiben — selbst wenn der
              // anschließende Episode-Match nicht greift, wird der Retry
              // verbucht und der Cooldown beginnt zu laufen.
              const nowIso = new Date().toISOString();
              for (const r of decision.eligible) {
                await admin
                  .from("graphiti_sync_log")
                  .update({ attempt: (r.attempt ?? 0) + 1, last_retry_at: nowIso })
                  .eq("entity_id", r.entity_id)
                  .eq("status", "failed");
              }
            }
          }
          log.stage("retry_select", "selected failed-row retries", retryStats);
        }
      }
      ...
      try {
        ...
        const cfIds = Array.from(new Set((stale ?? []).map((r) => r.entity_id))).filter(
          Boolean,
        ) as string[];
        if (cfIds.length > 0) {
          ...
          const flipIds = (stale ?? [])
            .filter((r) => confirmedIds.has(r.entity_id))
            .map((r) => r.id);
          if (flipIds.length > 0) {
        ...
        log.stage("stale_cleanup", "done", {
          staleFlipped,
          errorsCleared,
          scannedQueued: staleIds.length,
        });
      } catch (e) {
        log.warn("stale_cleanup", "failed (non-fatal)", {
      ...
      for (const [projectId, facts] of byProject) {
        ...
        for (const cf of facts) {
          ...
          if (!ep) {
            await logSync(admin, cf, "missing", {
              hint: "episode noch nicht im /episodes-Fenster",
            });
            projMissing++;
            missing++;
            continue;
      ...
      log.stage("done", "reconcile complete", {
