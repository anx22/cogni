# Character-System für die Entity

Ziel: Entity bekommt austauschbare „Charaktere". Erster neuer Charakter ist der **Face Pill** (Augen + rotierende Blur-Bälle, inspiriert von uiverse `Cobp/serious-mule-50`). Im aufgeklappten Zustand erscheinen **keine** Textarea/Checkbox, sondern vier schlichte Buttons für die Input-Modi (Notiz, Link, Datei, Sprache) im Look der Komponente. Auswahl des Charakters läuft erst mal nur im **Orb Lab**.

## Architektur (minimal, kein Overengineering)

```text
src/components/entity/
├── Entity.tsx                       (erweitert: nimmt characterId entgegen)
├── characters/
│   ├── types.ts                     Character-Interface
│   ├── registry.ts                  Liste aller Charaktere
│   ├── SiriCharacter.tsx            wrappt heutige SiriOrb + EntitySurface
│   └── FacePillCharacter.tsx        neuer Pill-Charakter (Augen, Blur-Bälle, Toggle)
```

`Character` ist bewusst klein:

```ts
interface Character {
  id: string;            // "siri" | "face-pill"
  label: string;
...
interface CharacterRenderProps {
  state: EntityState;
  size: number;
  sample: SampledPreset;       // bestehende Sampling-Pipeline
  onPickInputMode?: (mode: InputMode) => void;  // nur relevant für face-pill
  onClick?: () => void;
}
```

`Entity.tsx` macht künftig nur noch:

1. State-Management + Sampling (wie heute)
2. `registry[characterId].render({...})` aufrufen

Damit bleibt die bestehende Pipeline (Presets, Surface, Sample) unverändert; jeder Charakter zieht nur die Werte, die er braucht.

## Charakter 1 — `SiriCharacter` (Default)

Reine Extraktion: enthält das, was heute in `Entity.tsx` zwischen `<EntitySurface />` und `<SiriOrb />` steht. Keine Verhaltensänderung.

## Charakter 2 — `FacePillCharacter` (neu)

Visuell:

- **Pill-Form** (rounded-full, Höhe ≈ 40 % der Orb-Größe), zentriert.
- **4 weiche Blur-Bälle** rotieren auf einem Kreis hinter der Pill (`@keyframes rotate-balls 10s linear`), Farben aus `sample.colors.c1/c2/c3` + `bg`. Verwendet `filter: blur(...)` analog zu `SiriOrb`, damit das Theming dieselben Tokens nutzt.
- **Augen** (zwei kleine `oklch(...)`-Punkte), Blink-Animation alle ~6 s (`@keyframes blink`).
- **Reduziert** wenn `prefers-reduced-motion`.

Zustände:

- `idle` → Augen geradeaus, Bälle langsam.
- `hover` → Augen leicht hoch, Bälle etwas schneller.
- `processing` → Augen geschlossen, Pill atmet leicht.
- `review-ready` → Augen leuchten kurz auf.
- `failed` / `busy-blocked` → Augen halb-zu, Pill entsättigt (über `sample.colors`).

Toggle-Verhalten (statt Chat):

- Click auf Pill → expandiert zu größerer Pill (z. B. 280×120) per `transition-all`.
- Im offenen Zustand erscheinen **vier kleine Pill-Buttons** in einer Reihe: `Notiz · Link · Datei · Sprache` (Icons aus `lucide-react`: `FileText`, `Link`, `Paperclip`, `Mic`).
- Styling übernimmt **exakt** die existierenden `InputPills`-Klassen, damit das Theming konsistent ist (border-primary/40 bg-primary/10 text-primary, hover-Variante etc.) — wir reusen `InputPills` direkt, kein Klon.
- Click auf einen Button ruft `onPickInputMode(mode)` auf und kollabiert die Pill.
- **Kein** Textarea, **kein** Submit-Button, **keine** Voice-Aufnahme im Charakter selbst — die Pill ist nur ein Input-Mode-Picker.

## Orb Lab — Charakter-Auswahl

In `OrbLab.tsx` direkt über der Live-Orb-Card eine kleine `Tabs`-Leiste:

```text
[ Siri ]  [ Face Pill ]
```

State `characterId`, an `<Entity character={characterId} ... />` durchgereicht. Bei `face-pill`: `onPickInputMode` zeigt nur einen `toast.info("Input-Mode: …")`, damit die Mechanik testbar ist, ohne Intake-Pipeline zu berühren.

Die Editor-Sektion (Farben, Animation, Surface) bleibt **vollständig** — beide Charaktere greifen auf dasselbe Preset-Modell zurück. Der Face Pill ignoriert nur `surface.*`, was unkritisch ist.

## Was bewusst NICHT passiert

- Keine Persistenz der Charakter-Wahl in der DB (kommt später, wenn Mechanik bestätigt ist).
- Keine Änderung an `Index.tsx` / `HomeDropOverlay` / `InputOverlay`.
- Kein neues Preset-Schema pro Charakter — die bestehenden Ranges reichen.
- Kein Realtime/Settings-Eintrag, kein Migration.
- Keine neue Dependency.

## Geänderte / neue Dateien

- **neu** `src/components/entity/characters/types.ts`
- **neu** `src/components/entity/characters/registry.ts`
- **neu** `src/components/entity/characters/SiriCharacter.tsx`
- **neu** `src/components/entity/characters/FacePillCharacter.tsx`
- **edit** `src/components/entity/Entity.tsx` — delegiert an Registry, neuer `character?: CharacterId` Prop (default `"siri"`)
- **edit** `src/pages/OrbLab.tsx` — Charakter-Tabs + `onPickInputMode`-Handler

Damit ist die Mechanik sauber gekapselt, der bestehende Pfad (`siri`) bleibt 1:1 erhalten, und neue Charaktere lassen sich später durch eine einzige Datei + ein Eintrag in der Registry hinzufügen.  
--  
Exakte implementierung hier for reference:  
                ...
                <div className="options">
                  ...
                  <div className="btns-add">
                    ...
                    <button>
                      ...
                      <svg viewBox="0 0 24 24" height={20} width={20} xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)">
                    ...
                    <button>
                      ...
                      <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width={20} height={20} viewBox="0 0 24 24">
...
const StyledWrapper = styled.div`
  ...
  .background-blur-balls .[ball.green](http://ball.green) {
