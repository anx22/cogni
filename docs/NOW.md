...
- **Stufe 5 — Feedback-Schleife minimal schließen.** Resolver liest `link_rejected`-Events → schlägt denselben Falsch-Match nicht erneut vor. Akzeptierte Aliasse wachsen → deterministische Auflösung wird besser. Leichter als L1 (kein Prompt-Lernen — das bleibt zurückgestellt).
...
Reihenfolge: S1 → S2 (billig, sofort sichtbar) → S3 → S4 → S5 → S6. Owner-Entscheidungen: generische `entities`-Tabelle · Graphiti-getriebene Resolution mit lokalem Guard (siehe `DECISIONS.md` 2026-06-03).
