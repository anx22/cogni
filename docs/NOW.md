...
## Offener Backlog-Posten

> ⚠️ **EntityRail auf Home — BACKLOG, erst gemeinsam planen.**
> `EntityRail` gehört auf **ProjectScreen**, nicht auf Home. Home-Entität soll **zentral in der Mitte** bleiben. Die Verschiebung war ein Missverständnis. Nebenbefund: `EntityRail` ist `hidden lg:flex` → unter 1024 px auf Home **keine** Entität sichtbar.
> Rückbau mit nächstem Home-UI-Update — Platzierung/AssetOrbit/Composer vorher gemeinsam planen.
> Betrifft: `src/pages/Index.tsx` · `src/components/entity/EntityRail.tsx` · `ProjectScreen`.
...
- **2026-06-03 — M2 abgeschlossen** `EntityRail`, QA F3/R5 (`commitRoute` extrahiert + 17 Tests), Vercel-Import. 114/114 ✓, tsc 0.
