...
- [x] **Upload-Pipeline V1 (Phase 6)** — Auth (`/auth`, `useAuth`), privater Bucket `intake-files` mit pfadbasierter RLS, `useIntake` schreibt echt: Datei → Storage + assets + invoke `intake-process`; Notiz → `file_type=note` mit `metadata.kind=note`; Link → `file_type=other` mit `metadata.kind=url`. Edge Function ruft Unstructured, schreibt `parsed_documents` + `sources`, setzt Status. Realtime-Subscription auf `assets` spiegelt Status auf Entity-Kern. Rechtes SideGrid (`RecentAssets`) zeigt letzte 16 Inputs.
- [ ] Knowledge-Graph-Integration (Phase 7, Graphiti) + Proposed-Facts-Generierung
