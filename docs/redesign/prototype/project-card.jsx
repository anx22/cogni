...
function ProjectRow({ project, active = false, hover = false }) {
  return (
    <div style={{
      ...
      <span style={{
        flex: 1, fontSize: 13.5, color: active ? "var(--ink)" : "var(--ink-2)",
    ...
    <div className="card" style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontSize: 16, fontWeight: 500, letterSpacing: "-.015em",
      ...
      {!dense && (
        <>
          ...
          <div style={{ display: "flex", gap: 16, color: "var(--ink-3)", fontSize: 11.5 }}>
            ...
            <span style={{ marginLeft: "auto", color: dominant === "conflict" ? "var(--sig-conflict)" : "var(--ink-3)" }}>
              {dominant === "conflict" ? "Review fällig" :
               dominant === "review"   ? "Review fällig" :
               dominant === "action"   ? "In Bearbeitung" : "Ruhig"}
            </span>
          </div>
        </>
      )}
...
function ProjectHero({ project, lageLine, dense = false }) {
  const dominant = project.signals?.[0] || "calm";
  const stateLabel = dominant === "conflict" ? "Konflikt offen"
                    : dominant === "review"   ? "Review fällig"
                    : dominant === "action"   ? "Aktiv"
                    : "Ruhig";
  ...
  return (
    <div className="card" style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            color: "var(--ink-3)", fontSize: 11.5, letterSpacing: "0.02em",
            textTransform: "uppercase",
          }}>
            <span className={`dot dot--${dominant}${project.live ? " dot--live" : ""}`} style={{ position:"relative" }}/>
            <span style={{ color: `var(--sig-${dominant})`, fontWeight: 500 }}>{stateLabel}</span>
            <span style={{ color: "var(--ink-4)" }}>·</span>
            <span className="mono" style={{ textTransform: "none", letterSpacing: 0 }}>{project.last}</span>
          </div>
          <h3 style={{
            margin: "4px 0 0 0",
            fontSize: 22, fontWeight: 500, letterSpacing: "-.018em",
            color: "var(--ink)",
          }}>{project.name}</h3>
        </div>
        <button style={{
          background: "transparent", border: "1px solid var(--hair-2)", borderRadius: 8,
          width: 30, height: 30, color: "var(--ink-3)", cursor: "pointer",
      ...
      {!dense && (
        <div style={{
          marginTop: 16, display: "flex", gap: 6, flexWrap: "wrap",
        }}>
          {project.openCount > 0 && (
            <span className="chip"><span style={{color:"var(--ink-3)"}}>offen</span>
              <span className="mono tabular" style={{ marginLeft: 2 }}>{project.openCount}</span>
            </span>
          )}
