  ...
  return (
    <aside style={{
      ...
      <button className="btn btn--ghost" style={{
        marginTop: 8, justifyContent: "flex-start", padding: "0 10px",
        color: "var(--ink-3)", height: 32, fontSize: 13
      }}>
...
function PipelineWidget() {
  ...
  const stages = [
    ...
    { key: "review",   label: "Review fällig", count: 3, state: "waiting", color: "var(--sig-review)" },
    { key: "konflikt", label: "Konflikt",      count: 2, state: "waiting", color: "var(--sig-conflict)" },
  ];

  return (
    <div>
      {/* Live activity */}
      <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 10 }}>Jetzt</div>
      <div style={{
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 12, color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 6 }}>
            verstehe<span className="mono" style={{ color: "var(--ink-4)" }}>· 0:08</span>
          </div>
          <div style={{
            fontSize: 13, color: "var(--ink)", letterSpacing: "-.005em",
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
            marginTop: 2,
          }}>Steering-Protokoll v3</div>
      ...
      {/* Stages */}
      <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 10 }}>Pipeline</div>
          ...
          return (
            <div key={s.key} style={{
              ...
              <span className="mono tabular" style={{
                fontSize: 12, color: s.count > 0 ? accent : "var(--ink-4)",
      ...
      <div className="mono" style={{
        marginTop: 14, fontSize: 10.5, color: "var(--ink-4)", letterSpacing: ".02em",
      }}>
...
function ImpactPanel({ impacts }) {
  return (
    <aside style={{
      width: 290, padding: "32px 28px",
      borderLeft: "1px solid var(--hair)",
      flex: "0 0 auto",
      display: "flex", flexDirection: "column", gap: 22
    }}>
      <div>
        <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 12 }}>Letzter Impact</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {impacts.map((it, i) =>
          <div key={i} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <div className="mono" style={{ fontSize: 11, color: "var(--ink-4)" }}>{it.when}</div>
              <div style={{ fontSize: 14, lineHeight: 1.4, color: "var(--ink)", letterSpacing: "-.01em" }}>
                {it.what}
              </div>
              <div style={{
  ...
  return (
    <button
      ...
      <span style={{
        ...
        transition: "box-shadow .2s, border-color .2s", borderRadius: "50px"
      }}>
        <span style={{ transform: "scale(1.6)", display: "inline-flex" }}>{icon}</span>
      </span>
      <span style={{ fontSize: 12, color: hover ? "var(--ink-2)" : "var(--ink-3)", fontWeight: 500 }}>{label}</span>
    </button>);
  ...
  return (
    <>
      <div style={{ textAlign: "center", maxWidth: 720 }}>
        <h1 style={{
          margin: 0,
          fontSize: 48, lineHeight: 1.08, letterSpacing: "-.028em",
          fontWeight: 300, color: "var(--ink)"
        }}>Was gibt es neues?</h1>
        <p style={{
          margin: "10px 0 0",
          fontSize: 15, color: "var(--ink-3)", fontWeight: 400, letterSpacing: "-.005em"
        }}>Gebe mir Daten, tippe oder sprich. Ich ordne es ein.</p>
      </div>
...
function ActiveInputPaste() {
  return (
    <div style={{ width: "100%", maxWidth: 580, display: "flex", flexDirection: "column", alignItems: "stretch" }}>
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        marginBottom: 10
      }}>
        <div className="mono" style={{ fontSize: 11, color: "var(--ink-3)", letterSpacing: ".04em", textTransform: "uppercase" }}>
          Aus der Zwischenablage
        </div>
        <button className="btn btn--ghost" style={{ height: 24, padding: "0 8px", fontSize: 11, color: "var(--ink-4)" }}>verwerfen</button>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute", top: 0, right: 18, transform: "translateY(-50%)",
          padding: "3px 9px", borderRadius: 999, background: "var(--surface-1)",
          border: "1px solid var(--hair-2)",
          fontFamily: "Geist Mono, monospace", fontSize: 10, color: "var(--ink-3)",
          display: "flex", alignItems: "center", gap: 6
        }}>
          <span style={{ width: 8, height: 8, borderRadius: 999, border: "1.5px solid var(--sig-action)", borderTopColor: "transparent", animation: "cogni-orb-rotate-slow 1.2s linear infinite" }} />
          cogni liest…
        </div>
        <p style={{ margin: 0 }}>Hi Thomas,</p>
        <p style={{ margin: "8px 0" }}>
          kurze Info aus dem Steering heute: wir verschieben den Go-Live auf den
          <mark style={{ background: "var(--sig-action-soft)", color: "var(--accent-ink)", padding: "0 4px", borderRadius: 4 }}> 1. Juni 2026</mark>.
          Das deckt sich nicht mit der Mail von dir vom 9.04. — bitte mit
          <mark style={{ background: "var(--sig-action-soft)", color: "var(--accent-ink)", padding: "0 4px", borderRadius: 4 }}> Maria Schulz</mark> abstimmen, sie übernimmt jetzt die Projektsteuerung.
        </p>
        <p style={{ margin: "8px 0 0", color: "var(--ink-2)" }}>Beste Grüße<br />Jens</p>
      </div>
      ...
      <div style={{
        ...
        <span className="t-micro" style={{ color: "var(--ink-3)" }}>erkannt</span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--ink-2)" }}>
          <span style={{ width: 6, height: 6, borderRadius: 999, background: "var(--sig-action)" }} />
          Termin · 1. Juni
        </span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--ink-2)" }}>
          <span style={{ width: 6, height: 6, borderRadius: 999, background: "var(--sig-conflict)" }} />
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button className="btn btn--primary">Review öffnen <Ic.arrowR /></button>
        </div>
      </div>
    </div>);
...
function HomeScreen({ density = "full", state = "idle" }) {
  ...
  { type: "PDF", label: "Steering-Protokoll v3", age: "vor 1 min", status: "review-ready", ageRing: 0 },
  { type: "MAIL", label: "Berger · Go-Live", age: "vor 4 min", status: "understanding", ageRing: 0.2 }] :
  [
  { type: "PDF", label: "Steering-Protokoll v3", age: "vor 1 min", status: "review-ready", ageRing: 0 },
  { type: "MAIL", label: "Berger · Go-Live", age: "vor 4 min", status: "understanding", ageRing: 0.15 },
  ...
  const impacts = lean ? [
  { when: "vor 2 h", what: "Neuer Go-Live-Termin erkannt — Konflikt mit Steering-Protokoll", project: "Hafen Nord Erweiterung" },
  { when: "vor 25 min", what: "Maria Schulz als Stakeholder hinzugefügt", project: "Hafen Nord Erweiterung" }] :
  [
  { when: "vor 2 h", what: "Neuer Go-Live-Termin erkannt — Konflikt mit Steering-Protokoll", project: "Hafen Nord Erweiterung" },
  { when: "vor 25 min", what: "Maria Schulz als Stakeholder hinzugefügt", project: "Hafen Nord Erweiterung" },
  { when: "vor 4 h", what: "UX-Prototyp v2 ersetzt v1", project: "Aurora Rebrand" },
  { when: "vor 8 h", what: "Migrationsstrategie-Konflikt erkannt", project: "Hafen Nord Erweiterung" }];
