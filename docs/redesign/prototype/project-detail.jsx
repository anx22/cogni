...
function DetailSidebar({ activeId, projects, onCoreClick }) {
  ...
  return (
    <aside style={{
      ...
      borderRight: "1px solid var(--hair)", flex: "0 0 auto",
      ...
      <button
        ...
        <div style={{ minWidth: 0 }}>
          <div className="mono" style={{
            fontSize: 11.5, color: "var(--ink-2)", letterSpacing: "-.01em", fontWeight: 600,
          }}>cogni</div>
          <div className="mono" style={{
            fontSize: 10, color: hover ? "var(--accent)" : "var(--ink-4)",
            letterSpacing: ".04em", textTransform: "uppercase", marginTop: 2,
            transition: "color .15s",
          }}>{hover ? "öffnen ⌘ ␣" : "verstehe · 0:08"}</div>
        </div>
      </button>
      ...
      <div>
        <div style={{
          ...
          <span className="t-micro" style={{ color: "var(--ink-3)" }}>Projekte</span>
          <span className="mono tabular" style={{ fontSize: 11, color: "var(--ink-4)" }}>{projects.length}</span>
        </div>
    ...
    <section style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn--ghost"><Ic.upload /> Material</button>
          <button className="btn btn--primary"><Ic.spark /> Review öffnen<span style={{ marginLeft: 4 }}><Ic.arrowR /></span></button>
        </div>
      </div>

      {/* Lage-Etikett */}
      <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 14 }}>
        Lage <span style={{ color: "var(--ink-4)", textTransform: "none", letterSpacing: 0, marginLeft: 6 }}>
      ...
      <h1 style={{
        margin: 0,
        fontSize: 44, lineHeight: 1.04, letterSpacing: "-.03em", fontWeight: 500,
        color: "var(--ink)",
        maxWidth: 940,
      }}>{project.name}</h1>

      <p style={{
        margin: "20px 0 0", maxWidth: 880,
        fontSize: 24, lineHeight: 1.35, letterSpacing: "-.018em",
      ...
      <div style={{
        ...
        {project.lage.chips.map((c, i) => (
          <span key={i} className={`chip chip--${c.kind}`}>{c.label}</span>
        ))}
        <div style={{ flex: 1 }} />
        <div style={{ display: "flex", gap: 28, fontSize: 12.5, color: "var(--ink-3)" }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 6, color: "var(--ink)" }}>
              <span className="mono tabular" style={{ fontSize: 18, fontWeight: 500, letterSpacing: "-.02em" }}>
                {project.lage.naechsterTermin.date}
              </span>
              <span style={{ color: "var(--ink-3)", fontSize: 13 }}>{project.lage.naechsterTermin.topic}</span>
            </div>
          </div>
          <div>
            <div className="t-micro" style={{ marginBottom: 4 }}>Letzte Änderung</div>
            <div style={{ color: "var(--ink)", fontSize: 13.5 }}>{project.lage.letzteAenderung.what}</div>
            <div className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>{project.lage.letzteAenderung.rel}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <Ic.flag /> <span className="t-micro" style={{ color: "var(--ink-3)" }}>Outcome</span>
    ...
    <section style={{ padding: "32px 56px 0", flex: 1 }}>
      <header style={{
        ...
        <div style={{ display: "flex", gap: 4, fontSize: 12, color: "var(--ink-3)" }}>
          <button className="btn btn--ghost" style={{ height: 28, padding: "0 10px", fontSize: 12 }}>Alle</button>
          <button className="btn btn--ghost" style={{ height: 28, padding: "0 10px", fontSize: 12 }}>Nur Blocker</button>
          <button className="btn btn--ghost" style={{ height: 28, padding: "0 10px", fontSize: 12 }}>Ohne Frist</button>
        </div>
      </header>
          ...
          return (
            <div key={mode}>
              <div style={{
                ...
                <ModeMark mode={mode} />
                <span className="t-micro" style={{ color: meta.color, letterSpacing: ".06em" }}>{meta.label}</span>
                <span className="mono tabular" style={{ color: "var(--ink-4)" }}>{list.length}</span>
...
function ActionRow({ a, dense, last }) {
  return (
    <div style={{
      ...
      {a.blocker && (
        <span title="Blocker" style={{
          width: 3, height: 32, borderRadius: 2,
          background: "var(--sig-conflict)",
          flex: "0 0 auto", marginLeft: -8,
        }}/>
      )}
      <span title={a.kind} style={{
        width: 26, height: 26, borderRadius: 6,
        background: "var(--surface-2)", color: "var(--ink-3)",
        display: "flex", alignItems: "center", justifyContent: "center",
        flex: "0 0 auto",
      }}>
        {a.kind === "konflikt" ? <Ic.conflict /> :
         a.kind === "gap" ? <Ic.gap /> :
         a.kind === "feedback" ? <Ic.bolt /> :
         a.kind === "dependency" ? <Ic.link /> :
         <Ic.flag />}
      </span>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          fontSize: 14.5, color: "var(--ink)", fontWeight: 450, letterSpacing: "-.01em",
        }}>
        ...
        {a.owner && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <span style={{
              ...
              background: "var(--surface-3)", display: "inline-flex", alignItems: "center", justifyContent: "center",
              fontFamily: "Geist Mono, monospace", fontSize: 9, color: "var(--ink-2)",
            }}>{a.owner.split(" ").map(s => s[0]).join("").slice(0,2)}</span>
            <span style={{ color: "var(--ink-2)" }}>{a.owner.split(" ")[0]}</span>
          </span>
        )}
        {a.due && (
          <span className="mono tabular" style={{ color: "var(--ink-2)" }}>{a.due}</span>
        )}
        {!dense && (
          <span className="src" title={a.source}>{a.source}</span>
        )}
      </div>
    </div>
  );
...
function VerlaufFeed({ items, dense = false }) {
  ...
  return (
    <section style={{
      width: 340, padding: "32px 32px 0",
      borderLeft: "1px solid var(--hair)",
      flex: "0 0 auto",
    }}>
      <header style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 22 }}>
        <h2 style={{
          margin: 0, fontSize: 20, fontWeight: 500, letterSpacing: "-.018em",
          color: "var(--ink)",
        }}>Verlauf</h2>
        <span className="mono tabular" style={{ fontSize: 12, color: "var(--ink-3)" }}>{items.length}</span>
      </header>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "relative" }}>
        ...
        {list.map((e, i) => (
          <div key={e.id ?? i} style={{ display: "flex", gap: 14, position: "relative" }}>
            <div style={{
              width: 13, height: 13, borderRadius: 999, marginTop: 4,
              background: "var(--surface-1)",
              border: `2px solid ${e.delta === "widersprochen" ? "var(--sig-conflict)"
                                : e.delta === "neu" ? "var(--sig-action)"
                                : e.delta === "ersetzt" ? "var(--sig-review)"
                                : "var(--ink-3)"}`,
              flex: "0 0 auto", zIndex: 1,
            }}/>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span className="mono tabular" style={{ fontSize: 11, color: "var(--ink-3)" }}>{e.date}</span>
              ...
              {!dense && (
                <div className="src" style={{ marginTop: 3 }}>{e.source}</div>
              )}
            </div>
          </div>
        ))}
...
function SubstanzZone({ themen, documents, dense = false }) {
  return (
    <section style={{
      marginTop: 40, padding: "32px 56px 48px",
      ...
      <header style={{
        ...
        <h2 style={{
          margin: 0, fontSize: 18, fontWeight: 500, letterSpacing: "-.015em",
          color: "var(--ink-2)",
        }}>Substanz</h2>
        <span className="t-micro" style={{ color: "var(--ink-4)" }}>Themen · Dokumente</span>
      </header>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 36 }}>
        ...
        <div>
          <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 12 }}>
            Themen <span className="mono tabular" style={{ marginLeft: 6, color: "var(--ink-4)" }}>{themen.length}</span>
          </div>
          <div style={{
            display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: dense ? 4 : 8,
          }}>
            {themen.slice(0, dense ? 4 : 8).map((t, i) => <ThemaRow key={t.name} t={t} />)}
          </div>
        </div>
        ...
        <div>
          <div className="t-micro" style={{ color: "var(--ink-3)", marginBottom: 12 }}>
            Dokumente <span className="mono tabular" style={{ marginLeft: 6, color: "var(--ink-4)" }}>{documents.length}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {documents.slice(0, dense ? 3 : 5).map((d, i) => (
              <div key={i} style={{
                display: "flex", alignItems: "center", gap: 12,
                padding: "8px 0", borderBottom: i < documents.length - 1 ? "1px solid var(--hair)" : "none",
                fontSize: 13,
              }}>
                <span className="mono" style={{
                  fontSize: 10, padding: "2px 5px", borderRadius: 4,
                  background: "var(--surface-2)", color: "var(--ink-3)", letterSpacing: ".02em",
                  textTransform: "uppercase",
                }}>{d.typ}</span>
                <span style={{ flex: 1, color: "var(--ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</span>
                <span className="mono tabular" style={{ color: "var(--ink-3)", fontSize: 11 }}>v{d.v}</span>
                <span className="mono tabular" style={{ color: "var(--ink-4)", fontSize: 11, width: 44, textAlign: "right" }}>{d.date}</span>
              </div>
            ))}
          </div>
        </div>
...
function ThemaRow({ t }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 14, color: "var(--ink)", fontWeight: 450, letterSpacing: "-.01em" }}>{t.name}</div>
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", marginBottom: pasted ? 28 : 36 }}>
        ...
        <AssetOrbit
          items={[
            { type: "PDF",  label: "Steering v3",   age: "vor 1 min",  status: "review-ready",  ageRing: 0 },
            { type: "MAIL", label: "Berger",        age: "vor 4 min",  status: "understanding", ageRing: 0.2 },
            { type: "VOC",  label: "Memo · 1:24",   age: "vor 28 min", status: "review-ready",  ageRing: 0.6 },
          ]}
      ...
      {!pasted ? (
        <>
          <div style={{ textAlign: "center", maxWidth: 720 }}>
            <h1 style={{
              margin: 0,
              fontSize: 44, lineHeight: 1.08, letterSpacing: "-.028em",
              fontWeight: 300, color: "var(--ink)",
            }}>Was gibt es neues?</h1>
            <p style={{
              margin: "10px 0 0",
              fontSize: 14.5, color: "var(--ink-3)", fontWeight: 400, letterSpacing: "-.005em",
            }}>Material landet im globalen Intake — nicht automatisch in diesem Projekt.</p>
          </div>
